...
import java.util.Arrays;
    ...
    public final class OLL
    {
        ...
        private static void MoveOLL(FridrichSolver ScrambledCube, int[] YellowPositions)
        {
            int i, j, k, c;
            ...
            for (i = 0; i < 232; i++)
            {
                ...
                if (c == 8)
                    ...
                    {
                        case 1:
                            ScrambledCube.Solution += Constants.OLL_Algs[i / 4];
                            Tools.RotateCube(ScrambledCube, 7, i / 4);
                            return;
                        case 2:
                            ScrambledCube.Solution += Tools.yPerm(Constants.OLL_Algs[i / 4]);
                            Tools.RotateCube(ScrambledCube, 8, i / 4);
                            return;
                        case 3:
                            ScrambledCube.Solution += Tools.y2Perm(Constants.OLL_Algs[i / 4]);
                            Tools.RotateCube(ScrambledCube, 9, i / 4);
                            return;
                        case 0:
                            ScrambledCube.Solution += Tools.ydPerm(Constants.OLL_Algs[i / 4]);
